...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:1477/api/users/${userId}`, axiosConfig)
      ...
      .catch(() => alert('Session expired or unauthorized. Please login again.'));
  }, []);
  ...
  const updateProfile = async () => {
    ...
    try {
      if (avatarFile) {
        ...
        await axios.post(`http://localhost:1477/api/users/${userId}/avatar`, formData, {
          headers: { ...axiosConfig.headers, 'Content-Type': 'multipart/form-data' }
        });
      }

      await axios.put(`http://localhost:1477/api/users/${userId}/locations`, { homeLocation, workLocation }, axiosConfig);
      await axios.put(`http://localhost:1477/api/users/${userId}/preferred-regions`, { regions: preferredRegions.split(',').map(r => r.trim()) }, axiosConfig);
  ...
  const updateRoutes = async () => {
    ...
    try {
      await axios.put(`http://localhost:1477/api/users/${userId}/followed-routes`, { routes: followedRoutes.split(',').map(r => r.trim()) }, axiosConfig);
  ...
  return (
    <>
      <div className="menu">
        ...
        <a href="/user" className="user">User Profile</a>
        <a href="/dashboard" className="update">Update Profile</a>
