...
function UserDashboard() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:1477/api/users/${userId}`, {
  ...
  const fields = ['name', 'email', 'avatar', 'preferences', 'homeLocation', 'workLocation', 'preferredRegions'];
  let filledCount = 0;
        ...
        <div className="profile-details">
          ...
          <ul>
            {user.preferredRegions && user.preferredRegions.length > 0
              ? user.preferredRegions.map((region, idx) => <li key={idx}>{region}</li>)
